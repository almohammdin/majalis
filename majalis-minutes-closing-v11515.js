(()=>{
'use strict';
const VERSION=window.MAJALIS_VERSION||'1.15.15';
...
if(baseCapture&&!app.__minutesClosingCapture11515){
  app.captureSnapshot=()=>writeMode(baseCapture());
  app.__minutesClosingCapture11515=true;
}
const baseApply=app.applySnapshot?.bind(app);
if(baseApply&&!app.__minutesClosingApply11515){
  ...
  app.__minutesClosingApply11515=true;
}
const baseNew=app.startNewMeetingFromOrganization?.bind(app);
if(baseNew&&!app.__minutesClosingNew11515){
  ...
  app.__minutesClosingNew11515=true;
}
window.addEventListener('majalis:assistant-new-meeting',()=>{setNewMeeting();queueApply()});

if(!Storage.prototype.__majalisMinutesClosing11515){
  ...
  Storage.prototype.__majalisMinutesClosing11515=true;
}
...
// This wrapper is installed only after the voting/final modules are loaded.
const baseRender=window.renderDocuments;
if(typeof baseRender==='function'&&!baseRender.__minutesClosing11515){
  const wrapped=function(){const out=baseRender.apply(this,arguments);applyClosingMode();return out};
  wrapped.__minutesClosing11515=true;
...
// Safety net only; the source generator is also guarded in v1.15.15.
